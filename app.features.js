...
function settleExpiredPositions(){
  ...
  for(let i = list.length - 1; i >= 0; i--){
    ...
    // ✅ EVOLVE FEEDBACK (Upstash) — 서버로 성공/실패 기록 전송
    try{
      const tfMap = { "15":"15m", "30":"30m", "60":"1h", "240":"4h", "D":"1d", "W":"1w" };
      const tfSrv = tfMap[String(pos.tfRaw || pos.tf || "")] || String(pos.tf || "");
      const send = window.serverEvolveFeedback;
      if(typeof send === "function"){
        ...
        send({
          ...
          pnl: pnl/100,
          regime: pos.regime || "",
          meta: { patternKey: pos.patternKey || null }
        }).catch?.(()=>{});
      }
    }catch(e){}
...
async function executeAnalysisAll(){
  ...
  if(btn){
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> 서버 예측 중...';
  }

  try{
    checkCanceled(opToken);

    const symbol = (state.symbol || "BTCUSDT").toUpperCase();
    const api = window.serverPredict6tf;
    if(typeof api !== "function") throw new Error("SERVER_API_NOT_BOUND");

    const json = await api({ symbol });
    checkCanceled(opToken);
    ...
    for(const r of list){
      ...
      // explain(카드 렌더용): 기존 UI가 기대하는 형태로 맞춤
    ...
    showResultModalAll(symbol, map);
  }catch(e){
    if(String(e?.message || "").includes("CANCELLED")){
      toast("통합 예측이 취소되었습니다.", "warn");
      return;
    }
    console.error(e);
    toast("서버 예측 오류(통합 예측)", "danger");
  }finally{
    if(btn){
      btn.disabled = false;
      btn.innerHTML = '<i class="fa-solid fa-brain"></i> 통합 예측';
    }
...
async function quickAnalyzeAllAndShow(symbol){
  ...
  try{
    ...
    const api = window.serverPredict6tf;
    if(typeof api !== "function") throw new Error("SERVER_API_NOT_BOUND");
    const json = await api({ symbol: String(symbol||"").toUpperCase() });
    checkCanceled(opToken);
    ...
    showResultModalAll(String(symbol||"").toUpperCase(), map);
  }catch(e){
    if(String(e?.message || "").includes("CANCELLED")){
...
async function autoScanUniverseAll(opts={}){
  ...
  if(btn){
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> 서버 스캔 중...';
  }
  ...
  try{
    ...
    const api = window.serverScanAll;
    if(typeof api !== "function") throw new Error("SERVER_API_NOT_BOUND");
    ...
    console.error(e);
    toast("서버 스캔 오류(자동 스캔)", "danger");
    if(status) status.textContent = "오류";
  }finally{
    if(btn){
      btn.disabled = false;
      btn.innerHTML = '<i class="fa-solid fa-magnifying-glass"></i> 자동 스캔';
    }
    endOperation(opToken);
  }
}
...
/* === YOPO FIX: SERVER-ONLY ENGINE HOOKS (FINAL) ===
   - 지침 준수: 무거운 계산은 브라우저에서 절대 금지
   - 통합예측/스캔/백테스트는 server(Render) API만 호출
   - UI는 결과 표시만 담당
==================================================== */

function _need(fn, name){
...
window.executeAnalysisAll = async function(){
  ensureRuntimeState();
  const opToken = beginOperation("ANALYSIS_ALL_6TF_SERVER");
  const btn = document.getElementById("predict-all-btn");
  if(btn){ btn.disabled=true; btn.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> 통합 예측(6전략) 중...'; }
  try{
    checkCanceled(opToken);
    _need(window.serverPredict6tf, "serverPredict6tf");
    const symbol = state.symbol || state.activeSymbol || "BTCUSDT";
    const data = await window.serverPredict6tf({ symbol });
    const out = data?.resultsByTf || data?.results || data || {};
    showResultModalAll(symbol, out);
    toast("통합 예측(6전략) 완료", "success");
  }catch(e){
    if(String(e?.message||"").includes("CANCELLED")){ toast("진행 중 작업이 취소되었습니다.","warn"); return; }
    console.error(e);
    toast("통합 예측 실패 (서버/API 문제)", "danger");
  }finally{
    endOperation(opToken);
...
// 통합 스캔(20코인 × 6전략) — 서버 호출
window.autoScanUniverseAll = async function(){
  ensureRuntimeState();
  const opToken = beginOperation("SCAN_ALL_6TF_SERVER");
  ...
  try{
    checkCanceled(opToken);
    _need(window.serverScanAll, "serverScanAll");
    const data = await window.serverScanAll({});
    const perTf = data?.perTf || data?.resultsByTf || data?.byTf || {};
    if(perTf && typeof perTf === "object"){
    ...
    console.error(e);
    toast("통합 스캔 실패 (서버/API 문제)", "danger");
    if(status) status.textContent="통합 스캔 실패";
  }finally{
    endOperation(opToken);
...
async function openFromScanListOrSidebar(symbol){
  ...
  try{
    ...
    // 2) 없으면 서버 통합 예측(6TF) 호출 (브라우저 계산 금지)
    _need(window.serverPredict6tf, "serverPredict6tf");
    const data = await window.serverPredict6tf({ symbol });
    const out = data?.resultsByTf || data?.results || data || {};
    showResultModalAll(symbol, out);
...
async function runBacktest(){
  ...
  try{
    ...
    const api = window.serverBacktest;
    if(typeof api !== "function") throw new Error("SERVER_API_NOT_BOUND");
